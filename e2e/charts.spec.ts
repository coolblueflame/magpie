...
test('charts render from the seed, hover shows a tooltip, and every chart has a table twin', async ({ page }) => {
  ...
  await expect(page.getByTestId('chart-category-pick')).toHaveValue('cat_groc');   // first category in budget order
  await expect(page.getByTestId('chart-category-pick').locator('optgroup')).toHaveCount(2);
